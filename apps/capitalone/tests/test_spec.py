...
def test_provider_string_is_unchanged():
    ...
    assert storage.build_pdf_filename("2026-08-15", "August card statement - Savor (...1234)", "") == \
...
def test_csv_filenames_are_unchanged(tmp_path):
    ...
    assert paths.document_index_csv.name == "Capital One Document Index.csv"
