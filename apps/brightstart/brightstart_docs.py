...
from storage import (ALL_CATEGORIES, CONFIRMATION as CAT_CONFIRMATION,
                     PLAN as CAT_PLAN, STATEMENT as CAT_STATEMENT,
                     TAX as CAT_TAX)
log = logging.getLogger("brightstart_docs")
...
class Document:
    ...
    def __init__(self, title="", category="", summary="", date="", period="",
        ...
        self.kind = kind
        self.year = year
        self.cms_pdf = cms_pdf
        self.href_hint = href_hint
...
class App:
    def __init__(self, args):
        ...
        self.stats = {
            ...
            "discovered": 0, "statements": 0, "confirmations": 0,
            "tax_documents": 0, "plan_documents": 0,
    ...
    def page(self):
        ...
        if self._cdp_mode:
            ...
            brightstart = [p for p in live if site.is_safe_url(p.url or "")]
            brightstart = brightstart or [p for p in live if site.is_safe_url(p.url or "")]
    ...
    def cmd_login(self):
        print("Checking the connection to your signed-in Bright Start browser...\n")
        page = self.page()
        ...
        if challenge:
            ...
            print("Connected, but Bright Start shows a signed-out page.")
            ...
            print("Open Profile & Documents > Statements, Confirms & Tax Forms in that")
            print("browser, then run --diagnose.")
        self.close()
    ...
    def _in_scope(self, doc: Document) -> bool:
        ...
        if getattr(a, "account", None):
            want = str(a.account).strip()[-2:].zfill(2)
            if (doc.ext or "") != want:
                return False
    ...
    def _record_brightstart_doc(self, d: dict, ext_labels: dict) -> int:
        title = re.sub(r"\s+", " ", (d.get("label") or "")).strip()
        ...
        if category == CAT_STATEMENT:
            summary, confidence = "Statement", doc_types.HIGH
        elif category == CAT_PLAN:
            summary, confidence = summary or title or "Important Plan Information", doc_types.HIGH
        elif category == CAT_CONFIRMATION:

            summary, confidence = title or "Confirmation", doc_types.HIGH
        ...
        period = site.statement_period(date) if category == CAT_STATEMENT else ""
        doc = Document(title=title, category=category, summary=full_summary,
                       date=date, period=period,
                       confidence=confidence, account=account,
                       account_number=d.get("account_number") or "", ext=ext,
                       kind=d.get("kind") or "", year=d.get("year") or "",
    ...
    def cmd_discover(self, quiet: bool = False) -> int:
        ...
        if not site.ensure_statements(page):
            ...
            if not site.ensure_statements(page):
                print("Could not open your Bright Start documents page. Sign in in the")
    ...
    def download_one(self, page, doc: Document, filename: str):
        ...
        if doc.kind == site.KIND_INSERT:
            saved = site.download_insert(doc.cms_pdf, out_path)
        else:
            if not site.ensure_statements(page):
                self.check_session(page)
        ...
        elif doc.category == CAT_CONFIRMATION:
            self.stats["confirmations"] += 1
        elif doc.category == CAT_PLAN:
            self.stats["plan_documents"] += 1
    ...
    def cmd_run(self, mode_name: str):
        ...
        if mode_name == "all" and not self.args.yes:
            ...
            print(f"This downloads ALL available Bright Start documents ({scope}).")
    ...
    def cmd_diagnose(self):
        ...
        try:
            ...
            info["years"] = site.list_years(page)
            accounts = site.list_accounts(page)
            info["accounts"] = [site.redact_label(a["label"]) for a in accounts]

            per_year = []
            samples = []
            total = 0
            for year in info["years"]:
                html = site.fetch_list_html(page, year)
                if site.html_looks_signed_out(html):
                    per_year.append({"year": year, "signed_out": True})
                    continue
                rows = site.parse_rows(html)
                kinds = {}
                for r in rows:
                    cat = (site.CATEGORY_INSERT if r["kind"] == site.KIND_INSERT
                           else site.classify_row(r["kind"], r["label"]))
                    kinds[cat] = kinds.get(cat, 0) + 1
                    if len(samples) < 8:
                        samples.append({"label": r["label"][:60],
                                        "kind": r["kind"], "category": cat,
                                        "date": r["date"],
                                        "account": site.redact_label(r["account_number"]),
                                        "href_shape": re.sub(r"=[^&]+", "=...", r["href"])[:80]})
                total += len(rows)
                per_year.append({"year": year, "rows": len(rows), "by_category": kinds})
            info["listing"] = per_year
            info["listing_total"] = total
            info["samples"] = samples

            rendered = site.read_rendered_rows(page)
            info["rendered_rows_on_screen"] = len(rendered)
            info["rendered_row_labels"] = [r["label"][:60] for r in rendered[:8]]
            info["controls"] = [{"text": c[:60], "safe": site.is_safe_control(c)}
                                for c in site.read_controls(page)[:60]]
        ...
        for y in info.get("listing") or []:
            if y.get("signed_out"):
                print(f"  {y['year']}: LISTING LOOKS SIGNED OUT")
            elif y.get("rows"):
                print(f"  {y['year']}: {y['rows']} row(s)  {y['by_category']}")
        print(f"Listing total: {info.get('listing_total')}   "
              f"rendered on screen: {info.get('rendered_rows_on_screen')}")
        if info.get("error"):
    ...
    def write_run_summary(self):
        ...
        atomic_write_text(self.paths.run_summary, "\n".join([
            "Bright Start 529 Documents - run summary",
            "=" * 40,
            ...
            f"Confirmations downloaded:  {s['confirmations']}",
            f"Tax documents downloaded:  {s['tax_documents']}",
            f"Plan documents downloaded: {s['plan_documents']}",
...
def build_parser() -> argparse.ArgumentParser:
    ap = argparse.ArgumentParser(
        description="Local supervised Bright Start 529 document downloader (read-only)")
    ...
    ap.add_argument("--account", help="only this beneficiary sub-account "
                                      "(the two-digit extension, e.g. 01)")
