...
def test_provider_string_is_unchanged():
    ...
    assert storage.build_pdf_filename("2025-12-31", "Statement - Alex Sample (...01)", "") == \
        "2025-12-31 Bright Start 529 Statement - Alex Sample (...01).pdf"


def test_csv_filenames_are_unchanged(tmp_path):
    paths = storage.Paths(tmp_path)
    assert paths.document_index_csv.name == "Bright Start 529 Document Index.csv"
...
def test_precreated_folders_are_unchanged(tmp_path):
    ...
    assert made == ['Backups', 'Confirmations', 'Diagnostics', 'Logs',
                    'Manual Review', 'Plan Documents', 'Statements']
...
def test_plan_documents_are_filed_apart_from_statements(tmp_path):
    paths = storage.Paths(tmp_path)
    assert paths.folder_for(storage.PLAN) != paths.folder_for(storage.STATEMENT)
    assert paths.folder_for(storage.PLAN).name == "Plan Documents"
