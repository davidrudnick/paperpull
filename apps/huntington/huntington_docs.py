...
from storage import (ALL_CATEGORIES, CORRESPONDENCE as CAT_CORRESPONDENCE,
                     ESCROW as CAT_ESCROW, STATEMENT as CAT_STATEMENT,
                     TAX as CAT_TAX)
log = logging.getLogger("huntington_docs")
...
class App:
    def __init__(self, args):
        ...
        self.stats = {
            ...
            "discovered": 0, "statements": 0, "escrow_analyses": 0,
            "tax_documents": 0, "correspondence": 0,
    ...
    def page(self):
        ...
        if self._cdp_mode:
            ...
            huntington = [p for p in live if site.is_safe_url(p.url or "")]
            huntington = huntington or [p for p in live if site.is_safe_url(p.url or "")]
    ...
    def cmd_login(self):
        print("Checking the connection to your signed-in Huntington Bank browser...\n")
        page = self.page()
        ...
        if challenge:
            ...
            print("Connected, but Huntington Bank shows a signed-out page.")
            ...
            print("Open Service Center > View Statements and Documents in that browser, then run --diagnose.")
        self.close()
    ...
    def _record_huntington_doc(self, d: dict) -> int:
        ...
        elif category == CAT_ESCROW:
            summary, confidence = "Escrow Analysis", doc_types.HIGH
        elif category == CAT_CORRESPONDENCE:
            summary, confidence = "Correspondence", doc_types.HIGH
        elif category == CAT_TAX:
            if _cat != doc_types.TAX:
                summary, confidence = "Tax Form", doc_types.MEDIUM
        else:
            summary = summary or "Document"
        ...
        doc = Document(title=title, category=category, summary=full_summary,
                       ...
                       account_id=d.get("account_id") or "", last4=last4,
    ...
    def cmd_discover(self, quiet: bool = False) -> int:
        ...
        if not site.ensure_statements(page):
            ...
            if not site.ensure_statements(page):
                print("Could not open your Huntington statements page. Sign in in the")
    ...
    def download_one(self, page, doc: Document, filename: str):
        ...
        elif doc.category == CAT_ESCROW:
            self.stats["escrow_analyses"] += 1
        elif doc.category == CAT_CORRESPONDENCE:
            self.stats["correspondence"] += 1
    ...
    def cmd_run(self, mode_name: str):
        ...
        if mode_name == "all" and not self.args.yes:
            ...
            print(f"This downloads ALL available Huntington Bank documents ({scope}).")
    ...
    def cmd_diagnose(self):
        ...
        try:
            ...
            info["picker_identity"] = site.combobox_identity(page)
            info["picker_refused_as_money_control"] = site.is_money_control(info["picker_identity"])
            info["account_options"] = [site.redact_label(o) for o in site.account_options(page)]

            accounts = site.list_accounts(page)
            info["accounts_api"] = [{"label": site.redact_label(a["label"]),
                                     "product_type": a["product_type"],
                                     "status": a["status"], "enrolled": a["enrolled"]}
                                    for a in accounts]
            per_account = []
            total = 0
            for a in accounts:
                recs = site.list_records(page, a["account_id"])
                kinds = {}
                samples = []
                for r in recs:
                    cat, date, period, title = site.classify_record(r)
                    kinds[cat] = kinds.get(cat, 0) + 1
                    if len(samples) < 6:
                        samples.append({"record_type": r.get("recordType"),
                                        "title": title[:80], "category": cat,
                                        "date": date, "period": period,
                                        "fields": sorted(r.keys())})
                total += len(recs)
                per_account.append({"account": site.redact_label(a["label"]),
                                    "records": len(recs), "by_category": kinds,
                                    "samples": samples})
            info["records_api"] = per_account
            info["records_total"] = total

            rendered = []
            for a in accounts:
                if site.select_account(page, a["last4"]):
                    comp = site.read_component(page) or {}
                    rendered.append({"account": site.redact_label(comp.get("account") or ""),
                                     "tabs": comp.get("tabs"),
                                     "years": comp.get("years"),
                                     "rows_on_screen": len(comp.get("rows") or []),
                                     "row_titles": [r["title"] for r in (comp.get("rows") or [])[:8]]})
            info["rendered"] = rendered
            comp = site.read_component(page) or {}
            info["controls"] = [{"text": c[:60], "safe": site.is_safe_control(c)}
                                for c in (comp.get("controls") or [])[:60]]
        ...
        print(f"Accounts (API): {len(info.get('accounts_api') or [])}   "
              f"picker options: {len(info.get('account_options') or [])}")
        for a in info.get("records_api") or []:
            print(f"  {a['account']}: {a['records']} record(s)  {a['by_category']}")
        for r in info.get("rendered") or []:
            print(f"  rendered {r['account']}: tabs={[t['text'] for t in (r['tabs'] or [])]} "
                  f"rows on screen={r['rows_on_screen']}")
        if info.get("error"):
    ...
    def write_run_summary(self):
        ...
        atomic_write_text(self.paths.run_summary, "\n".join([
            "Huntington Bank Documents - run summary",
            "=" * 40,
            ...
            f"Escrow analyses downloaded:{s['escrow_analyses']}",
            f"Tax documents downloaded:  {s['tax_documents']}",
            f"Correspondence downloaded: {s['correspondence']}",
...
def build_parser() -> argparse.ArgumentParser:
    ap = argparse.ArgumentParser(
        description="Local supervised Huntington Bank document downloader (read-only)")
